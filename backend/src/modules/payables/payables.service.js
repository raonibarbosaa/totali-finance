...
async function list(tenantId, filters = {}) {
  ...
  const [data, total] = await Promise.all([
    prisma.payable_receivable.findMany({
      where,
      include: {
...
async function summary(tenantId) {
  ...
  const [po, ro, pod, rod] = await Promise.all([
    prisma.payable_receivable.aggregate({
      where: {
        tenant_id: tenantId,
        type: 'PAYABLE',
  ...
  return {
    payables: {
      total: Number(po._sum.amount || 0),
      count: po._count,
      overdue: Number(pod._sum.amount || 0),
      overdueCount: pod._count,
    },
    receivables: {
      total: Number(ro._sum.amount || 0),
      count: ro._count,
      overdue: Number(rod._sum.amount || 0),
      overdueCount: rod._count,
    },
  };
}

async function findOne(id, tenantId) {
  const r = await prisma.payable_receivable.findFirst({
    where: { id, tenant_id: tenantId },
    include: { category: true, bank_account: true, transaction: true },
  });
...
async function create(tenantId, userId, data) {
  ...
  return prisma.payable_receivable.create({
    data: {
      tenant_id: tenantId,
      type,
      description,
      ...
      notes: notes || null,
      competence_month: d.getMonth() + 1,
      competence_year: d.getFullYear(),
      created_by: userId,
    },
    include: { category: true, bank_account: true },
  });
}
...
async function update(id, tenantId, data) {
  ...
  return prisma.payable_receivable.update({
    where: { id },
    data: {
...
async function baixar(id, tenantId, userId, data) {
  ...
  const ba = await prisma.bank_account.findFirst({
  ...
  let domFields = {};
  if (r.category_id) {
    const c = await prisma.category.findUnique({ where: { id: r.category_id } });
    if (c) {
      domFields = {
        dominio_conta_debito: c.dominio_conta_debito,
        dominio_conta_credito: c.dominio_conta_credito,
        dominio_historico: c.dominio_historico,
        dominio_centro_custo_d: c.dominio_centro_custo_d,
        dominio_centro_custo_c: c.dominio_centro_custo_c,
      };
    }
  }
  ...
  const txn = await prisma.transaction.create({
    data: {
      tenant_id: tenantId,
      type: r.type === 'PAYABLE' ? 'EXPENSE' : 'INCOME',
      ...
      notes: notes || null,
      competence_month: pd.getMonth() + 1,
      competence_year: pd.getFullYear(),
      ...domFields,
      created_by: userId,
    },
  });
